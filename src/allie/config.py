...
`%APPDATA%\\allie\\`; the database and the logs belong under
`%LOCALAPPDATA%\\allie\\`, because a log file has no business following the
user to another computer through roaming profiles. `ALLIE_CONFIG_DIR`
redirects the first of those, which is how the tests avoid writing into the
developer's own settings.

**The product and the package are both `allie`** (plan.md D2; until 2026-09-22 the
product was `live-assistant` and the package `assistant`).
...
APP_TITLE = "Allie"
CONFIG_DIR_ENV = "ALLIE_CONFIG_DIR"
CONFIG_FILE_NAME = "config.toml"

# One entry per provider in the Credential Manager: service "allie",
...
def config_dir() -> Path:
    """The directory holding `config.toml` - `%APPDATA%\\allie` by default.
...
class TelegramSettings(BaseModel):
    ...
    `api_hash` beside it and the session `allie telegram login` produces
...
class Settings(BaseSettings):
    ...
    def settings_customise_sources(
        ...
        `model_config` so `ALLIE_CONFIG_DIR` still works when a test sets it
...
def is_configured() -> bool:
    """Whether setup has run. `allie run` refuses to start before it has."""
    return config_path().is_file() and bool(load_settings().live.primary)


_HEADER = (
    f"# {APP_NAME} settings. Written by `allie setup`; safe to edit by hand.\n"
...
def delete_api_key(provider: str) -> None:
    ...
    `allie purge` should not fail on a provider the user never used."""
