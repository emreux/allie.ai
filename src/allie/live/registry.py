...
from allie.config import load_api_key
from allie.live.base import LiveProvider
...
# one, and the wizard asks. The name is the old repository's until L2.1
# decides whether the OpenAI adapter is one of these.
...
def create_provider(
    ...
    *,
    ...
    user gave for an entry that has none of its own; given, it outranks the
    catalogue's.
    """
    ...
    if entry.requires_key and not key:
        raise MissingAPIKeyError(
            f"no API key stored for {provider_id!r} - run 'allie setup' to add one"
        )
    ...
    from allie.logs import keep_secret
...
def _read_catalog(path: Path | None) -> str:
    ...
    return (resources.files("allie") / "defaults" / CATALOG_FILE_NAME).read_text(encoding="utf-8")
