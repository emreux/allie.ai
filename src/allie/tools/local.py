...
repository at all: it is a `.py` file in `%APPDATA%\\allie\\tools\\`,
...
virtual environment: it imports `tool` from `allie.tools.registry`,
...
def local_tools_dir() -> Path:
    """`%APPDATA%\\allie\\tools` - beside `config.toml`."""
