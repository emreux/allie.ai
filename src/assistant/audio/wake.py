...
for the phrase (`assistant/wake/hey_friday.onnx`, F6a). It answers with a
...
def wake_model_path(name_or_path: str) -> Path:
    """The model file: a stem names one this product ships (`assistant/wake/`),
    ...
    if given.suffix.casefold() == ".onnx":
        ...
        return given
    shipped = Path(str(resources.files("assistant.wake") / f"{name_or_path}.onnx"))
    if not shipped.is_file():
        raise WakeModelMissingError(
            f"no wake-word model called {name_or_path!r} ships with this program "
            f"(looked for {shipped.name} beside assistant/wake/__init__.py)"
        )
    return shipped
